...
# 02/10/2018 - Versão 1.0
#
# Este script instala o Zabbix Server/Agent 4.0.X no Slackware 14.2. 
...
ZABBIX=`curl -s $URL_ZABBIX | grep sourceforge | grep 4.0.* | cut -d \" -f 2 | head -n 1 | sed -e "s/\/download//g"`
#
...
wget --quiet https://gitlab.com/eurodrigolira/slackware/raw/master/zabbix/4.0.x/rc.zabbix_server -O /etc/rc.d/rc.zabbix_server
chmod +x /etc/rc.d/rc.zabbix_server
#
echo -e "[\e[32m+\e[0m] Copiando o rc.zabbix_agent para o /etc/rc.d..."
wget --quiet https://gitlab.com/eurodrigolira/slackware/raw/master/zabbix/4.0.x/rc.zabbix_agent -O /etc/rc.d/rc.zabbix_agent
